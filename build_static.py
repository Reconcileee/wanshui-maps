...
产物结构 (部署后访问 https://<user>.github.io/wanshui-maps/tobu4th/):
  tobu4th/
    ...
    tiles/<snapshot_id>/0/<x>/<y>.webp  ← 由 map_source 转换而来 (offset 对齐, 无损 WebP)
...
瓦片格式采用无损 WebP (Pillow lossless=True), 相比 PNG 平均减小约 40% 体积,
加快 GitHub Pages 传输速度。前端 map.js 瓦片 URL 后缀为 .webp。
...
def _png_to_lossless_webp(src_png: Path) -> bytes:
    """PNG → 无损 WebP 字节串"""
...
def build():
    ...
    # 3. 瓦片: map_source → tiles/<id>/0/<x>/<y>.webp (按 offset 对齐网格, 无损 WebP)
    ...
    for snap in snapshots:
        ...
        for (mc_x, mc_z), src_path in snap["tiles"].items():
            tx = (mc_x - ox) // TILE_PX
            ty = (mc_z - oz) // TILE_PX
            dest = out_tile_dir / str(tx) / f"{ty}.webp"
            dest.parent.mkdir(parents=True, exist_ok=True)
            src_size = src_path.stat().st_size
            webp_bytes = _png_to_lossless_webp(src_path)
            dest.write_bytes(webp_bytes)
            total_src_bytes += src_size
            total_dst_bytes += len(webp_bytes)
            total_tiles += 1

    print(f"构建完成: {OUT_DIR}")
    print(f"  快照数: {len(snapshots)}")
    print(f"  瓦片总数: {total_tiles}")
    print(f"  PNG → 无损 WebP: {total_src_bytes/1024/1024:.1f} MB → "
          f"{total_dst_bytes/1024/1024:.1f} MB "
          f"(减小 {(1-total_dst_bytes/total_src_bytes)*100:.1f}%)")
    for snap in snapshots:
