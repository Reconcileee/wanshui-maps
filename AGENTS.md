...
13. **Cloudflare Pages 部署（推荐, 国内访问更快）**：除 GitHub Pages 外，项目同时部署到 Cloudflare Pages（访问地址 `https://wanshui-maps.pages.dev/`，**根路径，不带 /tobu4th**）。CF 在全球含亚太有 CDN 节点，国内访问速度优于 GH Pages。部署命令：`npx wrangler@latest pages deploy tobu4th --project-name wanshui-maps --commit-dirty=true --branch main`。首次需 `wrangler login` + `pages project create`。**关键坑点**：CF Pages 部署的是 `tobu4th/` 目录，其内容即为站点根，因此访问根路径；若带 `/tobu4th` 子路径，CF 会把所有请求（含 JSON/瓦片）fallback 到 index.html，导致 JS 拿到 HTML 而非 JSON，地图只显示按钮无瓦片。这与 GitHub Pages（需带 `/wanshui-maps/tobu4th` 子路径）不同，因 GH Pages 部署的是仓库根（含 tobu4th 子目录）。相对路径设计同时兼容两种部署的 index.html 内部资源引用。CF Pages 首次部署后可能出现短暂 522（CDN 同步中），等待 1-2 分钟后恢复。
